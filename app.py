...
if not st.session_state['autenticado']:
    ...
    aba_radar, aba_lupa, aba_stop = st.tabs(["📡 Radar (PM Dinâmico)", "🔬 Raio-X do Ativo", "🛡️ Radar (Alvo & Stop)"])
    ...
    # ==========================================
    # ABA 1: RADAR EM MASSA (PM DINÂMICO)
    # ==========================================
    with aba_radar:
        with st.sidebar:
    ...
    with aba_lupa:
        ...
        if btn_raiox:
            ...
            if not ativo_input:
                ...
                with st.spinner(f'Testando Backtest em {ativo}...'):
                    try:
                        df_full = tv.get_hist(symbol=ativo, exchange='BMFBOVESPA', interval=intervalo_tv, n_bars=5000)
