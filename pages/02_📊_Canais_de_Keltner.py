...
# ==========================================
# ABA 3: RADAR EM MASSA (ALVO & STOP LOSS KELTNER)
# ==========================================
with aba_stop:
    st.subheader("🛡️ Radar de Risco Definido (Alvo & Stop Loss - Keltner)")
    st.markdown("O robô faz apenas **uma** entrada ao tocar a Banda Inferior (sem preço médio) e aguarda o atingimento do Alvo ou do Stop de proteção.")
    
    st.markdown("##### ⚙️ Configurações da Varredura")
    cs1, cs2, cs3 = st.columns(3)
    with cs1:
        lista_stop = st.selectbox("Lista de Ativos:", ["BDRs Elite", "IBrX Seleção", "Todos (BDRs + IBrX)"], key="sk_lista")
        ...
        periodo_stop = st.selectbox("Período de Estudo:", options=['1mo', '3mo', '6mo', '1y', '2y', '5y', 'max'], format_func=lambda x: tradutor_periodo_nome[x], index=3, key="sk_per")
    with cs2:
        alvo_stop = st.number_input("Alvo de Lucro (%):", value=5.0, step=0.5, key="sk_alvo")
        perda_stop = st.number_input("Stop Loss (%):", value=5.0, step=0.5, key="sk_perda")
        keltner_mult_stop = st.number_input("Multiplicador Keltner:", min_value=0.5, max_value=10.0, value=3.0, step=0.1, key="sk_mult")
    with cs3:
        capital_stop = st.number_input("Capital por Trade (R$):", value=10000.0, step=1000.0, key="sk_cap")
        tempo_stop = st.selectbox("Tempo Gráfico:", ['15m', '60m', '1d', '1wk'], index=2, format_func=lambda x: {'15m': '15 min', '60m': '60 min', '1d': 'Diário', '1wk': 'Semanal'}[x], key="sk_tmp")
        
    btn_iniciar_stop = st.button("🚀 Iniciar Varredura (Alvo & Stop)", type="primary", use_container_width=True, key="sk_btn")
    ...
    if btn_iniciar_stop:
        ...
        for idx, ativo_raw in enumerate(ativos_stop):
            ...
            try:
                ...
                # --- CÁLCULO DO KELTNER ---
                kc = ta.kc(df_full['High'], df_full['Low'], df_full['Close'], length=20, scalar=keltner_mult_stop)
                if kc is None or kc.empty: continue
                coluna_inferior = [c for c in kc.columns if c.startswith('KCL')][0]
                df_full['Keltner_Inf'] = kc[coluna_inferior]
                df_full = df_full.dropna()
                ...
                for i in range(1, len(df_back)):
                    if em_pos:
                        # 1. Verifica primeiro o Stop Loss (Pior cenário)
                    ...
                    # CONDIÇÃO DE ENTRADA: Tocar na Banda Inferior
                    condicao_entrada = df_back['Low'].iloc[i] <= df_back['Keltner_Inf'].iloc[i]
                    if condicao_entrada and not em_pos:
                        em_pos = True
                        d_ent = df_back[col_data].iloc[i]
                        # A compra é feita exatamente no valor da Banda (limite)
                        preco_entrada = df_back['Keltner_Inf'].iloc[i]
                        take_profit = preco_entrada * (1 + alvo_dec)
                ...
                if em_pos:
                    ...
                    tem_sinal = df_full['Low'].iloc[-1] <= df_full['Keltner_Inf'].iloc[-1]
                    if tem_sinal: ls_sinais.append({'Ativo': ativo, 'Preço Atual': f"R$ {df_full['Close'].iloc[-1]:.2f}", 'Banda Inf': f"R$ {df_full['Keltner_Inf'].iloc[-1]:.2f}"})
        ...
        # --- EXIBIÇÃO DOS RESULTADOS ---
        st.subheader(f"🚀 Oportunidades Hoje (Radar Stop | Keltner {keltner_mult_stop:.1f})")
