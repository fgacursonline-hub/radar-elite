...
# AQUI ESTAVA O ERRO: Faltou esta linha para unificar a lista pro menu!
ativos_para_rastrear = sorted(list(set([a.replace('.SA', '') for a in (bdrs_elite + ibrx_selecao)])))

# ==========================================
# 2. CONFIGURAÇÃO DA PÁGINA & TVDATAFEED
# ==========================================
st.set_page_config(page_title="Bollinger Elite", layout="wide", page_icon="🔘")
...
st.markdown("Opere a volatilidade: Siga a Tendência (Walking up) ou cace as Reversões (Fechou Fora / Fechou Dentro).")
...
def calcular_bollinger(df, periodo=20, desvio=2.0, estrategia="Andando nas Bandas (Tendência)"):
    ...
    # Cálculo das Bandas usando pandas_ta
    bb = ta.bbands(df['Close'], length=periodo, std=desvio)
    if bb is None or bb.empty: return pd.DataFrame()
    
    # Captura nomes dinâmicos baseados nos parâmetros escolhidos
    col_u = [c for c in bb.columns if c.startswith('BBU_')][0]
    col_m = [c for c in bb.columns if c.startswith('BBM_')][0]
    col_l = [c for c in bb.columns if c.startswith('BBL_')][0]
    ...
    if "Andando nas Bandas" in estrategia:
        # Gatilho de Compra: Fecha ACIMA da banda superior
        df['Cruzou_Compra'] = (df['Close'] > df['BB_Upper']) & (df['Close'].shift(1) <= df['BB_Upper'].shift(1))
        # Saída do Indicador: Perde a Média Central (MMA20)
        df['Cruzou_Venda'] = (df['Close'] < df['BB_Mid']) & (df['Close'].shift(1) >= df['BB_Mid'].shift(1))
        
    elif "Fechou Fora / Fechou Dentro" in estrategia:
        # Lógica FFFD (Reversão para Média)
        fechou_fora_abaixo = df['Close'].shift(1) < df['BB_Lower'].shift(1)
        fechou_dentro = df['Close'] > df['BB_Lower']
        
        # Gatilho: Candle atual rompeu a banda pra baixo na barra anterior, mas agora fechou dentro
        df['Cruzou_Compra'] = fechou_fora_abaixo & fechou_dentro
        
        # Saída do Indicador: Tocou a Banda Central (Alvo 1 Clássico)
        df['Cruzou_Venda'] = (df['High'] >= df['BB_Mid'])
...
def exibir_explicacao_estrategia(estrategia):
    if "Andando" in estrategia:
        st.info("🌊 **Andando nas Bandas (Tendência):** Compra de força. Ocorre quando o candle fecha **ACIMA da banda superior**, indicando forte momentum comprador. O trade se mantém até atingir os alvos/stops ou até o preço perder a Banda Central (MMA20).")
    else:
        st.info("🪃 **Fechou Fora / Fechou Dentro (Reversão):** Caça ao pânico. Ocorre quando o preço fecha abaixo da banda inferior (desvio padrão) e o candle atual volta a fechar **DENTRO** das bandas. O alvo natural do indicador é a Banda Central (MMA20).")
...
with aba_radar:
    with st.container(border=True):
        ...
        with col_f2:
            estrategia_g = st.selectbox("Estratégia Bollinger:", [
                "1 - Andando nas Bandas (Tendência)", 
                "2 - Fechou Fora / Fechou Dentro (Reversão)"
            ], key="est_g")
        with col_f3:
        ...
        with col_p1:
            periodo_bb_g = st.number_input("Período da Média:", value=20, step=1, key="per_g")
        with col_p2:
            desvio_g = st.number_input("Desvios Padrão:", value=2.0, step=0.5, key="desv_g")
        with col_p3:
    ...
    btn_iniciar_global = st.button("🚀 Iniciar Varredura de Volatilidade", type="primary", use_container_width=True)
    ...
    if btn_iniciar_global:
        ...
        for i, ativo in enumerate(ativos_alvo):
            s_text.text(f"Medindo Volatilidade via TV: {ativo} ({i+1}/{len(ativos_alvo)})")
            p_bar.progress((i + 1) / len(ativos_alvo))
            try:
                df_full = tv.get_hist(symbol=ativo, exchange='BMFBOVESPA', interval=intervalo_tv, n_bars=5000)
                if df_full is None or len(df_full) < periodo_bb_g + 5: continue

                df_full.rename(columns={'open': 'Open', 'high': 'High', 'low': 'Low', 'close': 'Close'}, inplace=True)
                df_full = df_full.dropna()
                
                df_full = calcular_bollinger(df_full, periodo=periodo_bb_g, desvio=desvio_g, estrategia=estrategia_g)
                ...
                for j in range(len(df)):
                    ...
                    if trade_aberto is None:
                        if linha['Cruzou_Compra']:
                            if j == len(df) - 1:
                                dist_media = ((linha['Close'] / linha['BB_Mid']) - 1) * 100
                                oportunidades.append({"Ativo": ativo, "Sinal": "Gatilho Ativado 🟢", "Preço Atual": linha['Close'], "Dist. Média": f"{dist_media:.2f}%"})
                            else:
                ...
                if trade_aberto is not None:
                    ...
                    andamento.append({"Ativo": ativo, "Entrada": trade_aberto['entrada_data'].strftime("%d/%m/%Y"), "Dias": dias, "PM": trade_aberto['entrada_preco'], "Cotação Atual": df['Close'].iloc[-1], "Resultado Atual": resultado})
        ...
        st.subheader(f"🔘 Oportunidades Hoje ({estrategia_g.split('-')[1].strip() if '-' in estrategia_g else estrategia_g})")
        if oportunidades:
...
with aba_individual:
    with st.container(border=True):
        ...
        with c1:
            ...
            estrategia_rx = st.selectbox("Estratégia Bollinger:", [
                "1 - Andando nas Bandas (Tendência)", 
                "2 - Fechou Fora / Fechou Dentro (Reversão)"
            ], key="est_rx")
        with c2:
        ...
        with col_m1: periodo_bb_rx = st.number_input("Período da Média:", value=20, step=1, key="per_bb_rx")
        with col_m2: desvio_rx = st.number_input("Desvios Padrão:", value=2.0, step=0.1, key="desv_rx")
        with col_m3:
    ...
    btn_rx = st.button("🔍 Rodar Laboratório Bollinger", type="primary", use_container_width=True)
    
    if btn_rx:
        intervalo_tv_rx = tradutor_intervalo.get(tempo_rx, Interval.in_daily)
        with st.spinner(f"Calculando volatilidade de {ativo_rx} via tvDatafeed..."):
            try:
                df_full = tv.get_hist(symbol=ativo_rx.replace('.SA', ''), exchange='BMFBOVESPA', interval=intervalo_tv_rx, n_bars=5000)
                
                if df_full is not None and len(df_full) > periodo_bb_rx + 5:
                    df_full.rename(columns={'open': 'Open', 'high': 'High', 'low': 'Low', 'close': 'Close'}, inplace=True)
                    df_full = df_full.dropna()
                    
                    df_full = calcular_bollinger(df_full, periodo=periodo_bb_rx, desvio=desvio_rx, estrategia=estrategia_rx)
                    ...
                    if not df_ativo.empty:
                            ...
                            if em_aberto is None:
                                ...
                                if bateu_stop or bateu_alvo or sinal_venda:
                                    if bateu_stop:
                                        ...
                                        preco_saida = linha['Close']; motivo = "Saída do Indicador"
                        ...
                        if em_aberto is not None:
                            st.info(f"⏳ **{ativo_rx}: Em Operação** (Sinal Ativo | Desde {em_aberto['entrada_data'].strftime('%d/%m/%Y')} a R$ {em_aberto['entrada_preco']:.2f})")
                        else:
                            st.success(f"✅ **{ativo_rx}: Aguardando Novo Gatilho**")
                        ...
                        st.info("💡 **Dica:** No gráfico acima, clique em 'Indicadores' e adicione 'Bollinger Bands' para conferir as bordas de volatilidade visualmente.")
                else:
                    st.error("Sem dados suficientes para calcular as Bandas de Bollinger via tvDatafeed.")
            except Exception as e:
