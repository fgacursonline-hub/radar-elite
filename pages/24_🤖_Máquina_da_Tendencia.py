...
def ind_trend(df, di_len=13, adx_len=8, st_len=10, st_mult=3.0, smooth='rma'):
    if df is None or len(df) < 50: return None
    df.index = df.index.tz_localize(None)
    
    # O Pulo do Gato: Permite trocar RMA, SMA ou EMA para igualar ao Profit
...
st.title("🤖 Máquina de Tendência (Sincronizada)")
st.info("🟢 **Gatilho Cativado:** Só compra no EXATO DIA em que ADX cruza DI- para cima (Se DI+ > DI- e ST=Verde).")
...
with ab1:
    with st.container(border=True):
        ...
        adx_g = c3.number_input("ADX Per:", value=8, key="adx1")
        suav_g = c3.selectbox("Suavização ADX:", ['rma', 'sma', 'ema'], key="sv1")
        
        st_l_g = c4.number_input("ST Per:", value=10, key="st1")
        st_m_g = c4.number_input("ST Mult:", value=3.0, step=0.1, key="stm1")
        
    with st.container(border=True):
        st.markdown("🛡️ **Gestão de Risco**")
        c5, c6, c7, c8 = st.columns(4)
        u_al = c5.toggle("🎯 Alvo", value=True, key="ua1")
        al_g = c5.number_input("Alvo %:", value=15.0, disabled=not u_al, key="al1")
        u_st = c6.toggle("🛡️ Stop", value=False, key="us1")
        st_g = c6.number_input("Stop %:", value=5.0, disabled=not u_st, key="stp1")
        u_rst = c7.toggle("📉 Saída Rev ST", value=True, key="rst1")
        u_rdm = c8.toggle("📉 Saída Rev DMI", value=False, key="rdm1")
    ...
    if st.button("🚀 Iniciar Varredura", type="primary", use_container_width=True):
        ...
        for idx, atv in enumerate(lst):
            ...
            try:
                ...
                df_f = ind_trend(df_f, di_g, adx_g, st_l_g, st_m_g, suav_g)
...
with ab2:
    with st.container(border=True):
        ...
        suav_rx = c3.selectbox("Suavização ADX:", ['rma', 'sma', 'ema'], key="sv2")
        
        st_l_rx = c4.number_input("ST Per:", value=10, key="st2")
        st_m_rx = c4.number_input("ST Mult:", value=3.0, step=0.1, key="stm2")
        
        c5, c6, c7, c8 = st.columns(4)
        u_al_rx = c5.toggle("🎯 Alvo", value=True, key="ua2")
        al_rx = c5.number_input("Alvo %:", value=15.0, step=0.5, disabled=not u_al_rx, key="al2")
        u_st_rx = c6.toggle("🛡️ Stop", value=False, key="us2")
        st_rx = c6.number_input("Stop %:", value=5.0, step=0.5, disabled=not u_st_rx, key="st_2")
        ur_st_rx = c7.toggle("📉 Saída Rev ST", value=True, key="rst2")
        ur_dm_rx = c8.toggle("📉 Saída Rev DMI", value=False, key="rdm2")
        ...
        with st.spinner(f'Calculando {atv_rx}...'):
            try:
                ...
                if df_f is not None and len(df_f)>50:
                    df_f.rename(columns={'open':'Open','high':'High','low':'Low','close':'Close'}, inplace=True)
                    df_f = ind_trend(df_f, di_rx, adx_rx, st_l_rx, st_m_rx, suav_rx)
                    
                    dt_c = df_f.index[-1] - pd.DateOffset(months=int(per_rx.replace('mo','').replace('y','')) * (1 if 'mo' in per_rx else 12)) if per_rx != 'max' else df_f.index[0]
                    df_b = df_f[df_f.index >= dt_c].copy().reset_index()
...
with ab3:
    ...
    f_suav = cf2.selectbox("Suav. ADX F:", ['rma', 'sma', 'ema'])
    
    f_st = cf3.number_input("ST Per F:", value=10)
    f_stm = cf3.number_input("ST Mult F:", value=3.0, step=0.1)
    f_alv = cf3.number_input("Alvo Pts:", value=300 if "WIN" in f_sel else 10, step=50)
    
    f_cont = cf4.number_input("Contratos:", value=1)
    f_mlt = cf4.number_input("R$/Ponto:", value=0.20 if "WIN" in f_sel else 10.0)
    f_zer = cf4.checkbox("⏰ Zerar Fim Dia", value=True)
    f_sdm = cf4.checkbox("📉 Saída DMI", value=False)
    ...
    if st.button("🚀 Gerar Futuros", type="primary", use_container_width=True):
        with st.spinner('Simulando...'):
            try:
                ...
                if df_f is not None:
                    df_f.rename(columns={'open':'Open','high':'High','low':'Low','close':'Close'}, inplace=True)
                    df_f = ind_trend(df_f, f_di, f_adx, f_st, f_stm, f_suav)
