...
st.title("📉 Máquina Quantitativa: CCI (Commodity Channel Index)")
st.markdown("Identifique força direcional, reversões e momentum explosivo com a bússola de Donald Lambert.")
...
with aba_radar:
    ...
    btn_iniciar_global = st.button("🚀 Iniciar Varredura do CCI", type="primary", use_container_width=True)
    ...
    if btn_iniciar_global:
        ...
        st.subheader("🏆 Top 20 Histórico (Ranking do CCI)")
        if historico:
...
with aba_individual:
    ...
    if btn_rx:
        with st.spinner(f"Dissecando as ondas de {ativo_rx}..."):
