...
# ==========================================
# ABA 1: RADAR PADRÃO (COM STOP OPCIONAL E GATILHO VARIÁVEL)
# ==========================================
with aba_padrao:
    st.subheader("📡 Radar Padrão (Entrada Única)")
    st.info("📊 **A Estratégia Clássica (Rastreador IFR):** Busca exaustão da força vendedora. \n\n🟢 **Gatilho de Compra:** Ocorre quando a linha do IFR mergulha na região de pânico e, em seguida, **cruza de volta para cima**, confirmando que a venda exauriu e o repique começou. \n\n🔴 **Condução (Padrão):** O robô faz apenas uma entrada por sinal e sai estritamente no alvo de lucro fixo percentual ou no Stop Loss (se ativado).")
    ...
    with cp3:
        ...
        niv_p = st.number_input("Nível IFR (<):", value=25.0, step=1.0, key="p_niv")
            ...
            try:
                ...
                for i in range(1, len(df_back)):
                    if em_pos:
                        ...
                        continue
                    if df_back['IFR_Prev'].iloc[i] < niv_p and df_back['IFR'].iloc[i] >= niv_p:
                ...
                elif df_full['IFR_Prev'].iloc[-1] < niv_p and df_full['IFR'].iloc[-1] >= niv_p:
...
with aba_pm:
    ...
    with cr3:
        ...
        niv_pm = st.number_input("Nível IFR (<):", value=25.0, step=1.0, key="pm_niv")
        ...
        for idx, ativo_raw in enumerate(ativos_pm):
            ...
            try:
                ...
                for i in range(1, len(df_b)):
                    ...
                    if df_b['IFR_Prev'].iloc[i] < niv_pm and df_b['IFR'].iloc[i] >= niv_pm:
                        em_pos = True
                ...
                elif df['IFR_Prev'].iloc[-1] < niv_pm and df['IFR'].iloc[-1] >= niv_pm:
...
with aba_individual:
    ...
    with c2:
        ...
        rx_ifr = st.number_input("IFR Período:", value=8, key="rx_ifr")
    with c3:
        rx_stop = st.number_input("Stop Loss (%):", value=0.0, step=0.5, key="rx_stop") if rx_est == "Padrão" else 0.0
        rx_niv = st.number_input("Nível IFR (<):", value=25.0, step=1.0, key="rx_niv")
    ...
    if st.button("🔍 Rodar Análise", type="primary", use_container_width=True):
        try:
            ...
            # Aqui é apenas um feedback de sucesso e plotagem visual na versão simplificada
            st.success(f"Análise base de {rx_at} concluída. Gatilho definido em {rx_niv}.")
...
with aba_futuros:
    ...
    with f2:
        f_tmp = st.selectbox("Tempo Gráfico:", ['15m', '60m', '1d'], index=0, key="f_tmp")
    with f3:
        fut_niv = st.number_input("Nível IFR (<):", value=25.0, step=1.0, key="fut_niv")
        
    if st.button("🚀 Analisar Futuros", type="primary"):
        df = puxar_dados_blindados(f_at, tempo_grafico=f_tmp, barras=5000)
        st.write(f"Dados de {f_at} processados usando nível {fut_niv}.")
