...
def calcular_ichimoku_tk(df, tenkan=20, kijun=60, senkou=120, displacement=30, ema_p=200):
    ...
    # Donchian flexível (min_periods=1 evita retornar vazio em históricos curtos)
    def donchian(len_p):
        max_h = df['high'].rolling(window=len_p, min_periods=1).max()
        min_l = df['low'].rolling(window=len_p, min_periods=1).min()
        return (max_h + min_l) / 2

    df['tenkan_sen'] = donchian(tenkan)
    df['kijun_sen'] = donchian(kijun)
    df['ma_filtro'] = ta.ema(df['close'], length=ema_p).bfill() # bfill ajuda a não perder linhas iniciais
...
# 3. INTERFACE
# ==========================================
st.title("☁️ Ichimoku Cloud + Filtro")
...
with st.container(border=True):
    ...
    p_ema = c5.number_input("Média Móvel Filtro:", 200)

aba_radar, aba_test = st.tabs(["📡 Radar", "🔬 Backtest"])
    ...
    if st.button("🚀 Iniciar Escaneamento", type="primary", use_container_width=True):
        ...
        for idx, ativo in enumerate(ativos_tr):
            ...
            try:
                ...
                # Se der erro em um ativo, ele agora avisa qual erro foi
                st.warning(f"Erro em {ativo}: {e}")
        
        s_text.empty(); p_bar.empty()
        if ls_res:
            st.dataframe(pd.DataFrame(ls_res), use_container_width=True, hide_index=True)
        else:
            st.info("Nenhum resultado encontrado. Tente aumentar o histórico no seu motor_dados.")

# (Código do Backtest segue a mesma lógica de limpeza de colunas)
