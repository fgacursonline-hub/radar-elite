...
# ==========================================
# ABA 1: RADAR PADRÃO (CRUZAMENTO & ALVO)
# ==========================================
with aba_padrao:
    st.subheader("📡 Radar Padrão (Entrada no Cruzamento & Alvo Fixo)")
    st.markdown("O robô compra no exato momento em que a Média Curta cruza a Média Longa para cima e aguarda o alvo.")
    ...
    with cp2:
        ...
        tipo_cruzamento = st.selectbox("Estratégia de Médias:", ["Welles Wilder (8 x 21)", "Exponencial (16 x 42)"], key="cm_tipo")
    with cp3:
    ...
    if btn_iniciar_cm:
        ...
        for idx, ativo_raw in enumerate(ativos_cm):
            ...
            s_text.text(f"🔍 Analisando Cruzamentos: {ativo} ({idx+1}/{len(ativos_cm)})")
            ...
            try:
                ...
                # --- CÁLCULO DAS MÉDIAS (Welles Wilder ou MME) ---
                if tipo_cruzamento == "Welles Wilder (8 x 21)":
                    # RMA é a Welles Wilder Smoothing Moving Average no pandas_ta
                    df_full['Curta'] = ta.rma(df_full['Close'], length=8)
                    df_full['Longa'] = ta.rma(df_full['Close'], length=21)
                else:
                    df_full['Curta'] = ta.ema(df_full['Close'], length=16)
                    df_full['Longa'] = ta.ema(df_full['Close'], length=42)
                ...
                if em_pos:
                    ...
                    if cruzou_hoje: 
                        ls_sinais.append({'Ativo': ativo, 'Preço Atual': f"R$ {df_full['Close'].iloc[-1]:.2f}", 'Média Curta': f"{df_full['Curta'].iloc[-1]:.2f}", 'Média Longa': f"{df_full['Longa'].iloc[-1]:.2f}"})
        ...
        st.subheader(f"🚀 Cruzamentos de Alta Hoje ({tipo_cruzamento})")
        if len(ls_sinais) > 0: st.dataframe(pd.DataFrame(ls_sinais), use_container_width=True, hide_index=True)
        else: st.info("Nenhum ativo cruzou as médias para cima no último candle.")
